#!/usr/bin/env python3
"""Genera figura comparativa de val_loss desde summary o metrics reales."""
import argparse
import csv
import sys
...
def load_metrics(root: Path) -> List[Dict[str, str]]:
    ...
    for metrics_path in sorted((root / "experiments").glob("*/metrics.csv")):
        ...
        if best is None:
            continue
        rows.append({"exp_id": metrics_path.parent.name, "val_loss": best.get("val_loss", "")})
    return rows
...
def main() -> int:
    ...
    if not rows:
        print("ERROR: no hay datos reales para val_loss", file=sys.stderr)
        return 2
